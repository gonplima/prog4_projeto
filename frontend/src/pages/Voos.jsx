...
export default function Voos() {
  ...
  const [showForm, setShowForm] = useState(false); // controlar visibilidade do form
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        setEditId(null);
        setShowForm(false);  // esconder o form após salvar
        carregar();
      } else {
  ...
  function handleEdit(v) {
    ...
    setEditId(v.id);
    setShowForm(true); // mostrar form ao editar
    setMsg("");
  }
        ...
        {showForm && (
          ...
          >
            ...
            <div className="flex gap-3 mt-2">
              <button
                className="bg-blue-600 text-white rounded p-2 flex-1"
                type="submit"
              >
                {editId ? "Salvar Alterações" : "Cadastrar Voo"}
              </button>
              <button
                type="button"
                onClick={() => {
